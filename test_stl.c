#include <math.h>
#include <stdio.h>

#include "stl.h"

int main(int argc, char **argv) {
  if (argc < 2) {
    puts("Usage: test_stl path_to_stl.stl");
    return 1;
  }
  Buffer *triangle_buffer = read_stl(argv[1]);
  if (triangle_buffer == NULL) {
    puts("Failed to read STL");
    return 1;
  }

  /* Print mesh's bounding box */
  float bb_min[3] = {INFINITY};
  float bb_max[3] = {-INFINITY};

  STLTriangle *tri = NULL;
  for (size_t i = 0; i < triangle_buffer->num_appended_bytes / sizeof(STLTriangle); i++) {
    tri = ((STLTriangle *)triangle_buffer->mem_start) + i;
    float *v = tri->vertices[i];
    for (int j = 0; j < 3; j++) {
      bb_min[j] = fminf(v[j], bb_min[j]);
      bb_max[j] = fmaxf(v[j], bb_max[j]);
    }
  }
  printf("Number of triangles: %lu\n", triangle_buffer->num_appended_bytes / sizeof(STLTriangle));
  printf("Bounding Box: Max(%f, %f, %f), Min(%f, %f, %f)\n", bb_max[0], bb_max[1], bb_max[2], bb_min[0], bb_min[1],
         bb_min[2]);

  buffer_free(&triangle_buffer);
  triangle_buffer = NULL;
  return 0;
}